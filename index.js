...
function processSum(numberList, cb) {
  const sumOfNumberList = numberList.reduce((accumulator, currentValue) => {
    return accumulator + currentValue;
  }, 0);
  return cb(sumOfNumberList);
}
...
function processContains(item, list, cb) {
  let checkList = list.includes(item)
  return cb(checkList);
}
...
function firstNamesAllCaps(runners) {
  const allCaps = runners.map((names) => {
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const runnersWithTSize = runners.filter((items) => {
    return items.shirt_size == tShirtSize;
  });
  return runnersWithTSize;
}
...
function tallyUpDonations(runners) {
  const sumOfDonations = runners.reduce((acc, curVal) => {
